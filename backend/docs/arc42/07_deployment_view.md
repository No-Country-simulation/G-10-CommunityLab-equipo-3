...
3. **VCN (OCI)**: Compute Instance con el backend Java en **contenedor** → HTTPS → **OCI Object Storage** (paquetes `< 1MB`); salida a internet por IGW hacia el **proveedor de IA** (OpenAI), conexión persistente Gateway hacia **Discord** (bot JDA) y long polling hacia **Telegram** (bot TelegramBots), ambos con token por env.
